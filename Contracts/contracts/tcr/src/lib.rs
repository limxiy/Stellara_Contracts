...
impl TCRContract {
    pub fn apply(env: Env, applicant: Address, deposit: i128, metadata: Symbol) -> u32 {
        ...
        let mut listing_count: u32 = env.storage().instance().get(&symbol_short!("L_COUNT")).unwrap_or(0);
        listing_count += 1;
        ...
        env.storage().instance().set(&symbol_short!("L_COUNT"), &listing_count);
    ...
    pub fn challenge(env: Env, challenger: Address, listing_id: u32, deposit: i128) -> u32 {
        ...
        assert!(listing.status == ListingStatus::Pending, "Cannot challenge non-pending listing");
        assert!(deposit >= listing.deposit, "Insufficient challenge deposit");
        
        let mut challenge_count: u32 = env.storage().instance().get(&symbol_short!("C_COUNT")).unwrap_or(0);
        challenge_count += 1;
        ...
        env.storage().instance().set(&symbol_short!("C_COUNT"), &challenge_count);
    ...
    pub fn vote(env: Env, voter: Address, challenge_id: u32, side: bool, amount: i128) {
        ...
        let mut challenge: Challenge = env.storage().instance().get(&challenge_id).expect("Challenge not found");
        assert!(!challenge.resolved, "Challenge already resolved");
        assert!(env.ledger().timestamp() < challenge.end_time, "Voting period ended");
    ...
    pub fn resolve(env: Env, listing_id: u32) {
        ...
        if listing.status == ListingStatus::Challenged {
            let mut challenge: Challenge = env.storage().instance().get(&listing.challenge_id).expect("Challenge not found");
            assert!(env.ledger().timestamp() >= challenge.end_time, "Voting period active");
            ...
            env.storage().instance().set(&listing.challenge_id, &challenge);
        } else if listing.status == ListingStatus::Pending {
            assert!(env.ledger().timestamp() >= listing.expiry, "Challenge period active");
            listing.status = ListingStatus::Approved;
        }
